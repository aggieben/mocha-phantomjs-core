...
var tests = system.args[2];
var reporter = system.args[3] || 'spec';
var config = JSON.parse(system.args[4] || '{}');
var configured = false;
var runStarted = false;
var hookData;

var _usage = 'Usage: phantomjs mocha-phantomjs-core.js URL TESTS [REPORTER] [CONFIG-AS-JSON]';

if (!url) {
    system.stdout.writeLine('No url provided.\n' + _usage);
    phantom.exit(-1);
}

if (!tests) {
    system.stdout.writeLine('No test script provied.\n' + _usage);
    phantom.exit(-2);
}
...
page.onError = function(msg, traces) {
    if (page.evaluate(function() { return !!window.onerror })) {
        return;
    }
...
function configureMocha() {
    page.evaluate(function(conf, env, columns) {
        Mocha.reporters.Base.window.width = columns
        mocha.env = env

        mocha.useColors(conf.useColors)
        mocha.bail(conf.bail)
        if (conf.timeout) {
            mocha.timeout(conf.timeout)
        }
        if (conf.grep) {
            mocha.grep(conf.grep)
        }
        if (conf.invert) {
            mocha.invert()
        }
    ...
    if (page.evaluate(setupReporter, reporter) !== true) {
        ...
        wrapper = function() {
            ...
            require = function(what) {
                ...
                throw new Error('Your custom reporter tried to require "' +
                    what + '", but Mocha is not running in Node.js in mocha-phantomjs, so Node modules cannot be required - only other reporters');
            };
            module = {};
            exports = undefined;
        ...
        wrappedReporter = wrapper.toString().replace("'customreporter'", "(function() {" + (customReporter.toString()) + "})()"); // eslint-disable-line quotes

        page.evaluate(wrappedReporter);
        if (page.evaluate(function() { return !Mocha.reporters.Custom }) || page.evaluate(setupReporter) !== true) {
...
function setupReporter(rptr) {
    try {
        mocha.setup({
            reporter: rptr || Mocha.reporters.Custom
        })
        return true
    } catch (error) {
        return error
